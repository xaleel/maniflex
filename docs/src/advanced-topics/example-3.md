...
queueing payment — all in one transaction. An [action endpoint](actions.md)
...
satellite from `jobs/redis` (see [Events & Background Jobs](events-jobs.md))
...
[actions](actions.md), [raw queries](raw-queries.md),
[batch-saga](batch-saga.md), and
[events-jobs](events-jobs.md).
