...
| `Middleware`        | pipeline middleware scoped to this model, installed at registration — see [Writing Middleware](middleware.md)            |
...
| `ExportEnabled`     | mount `GET /:model/export` (CSV / XLSX) — see [CSV / XLSX Export](advanced/export.md)                                    |
| `MaxExportRows`     | row cap for the export endpoint; default 100,000                                                                         |
| `AggregateEnabled`  | mount `GET /:model/aggregate` (grouped count/sum/avg/min/max) — see [Aggregations](advanced/raw-queries.md#auto-generated-aggregate-endpoint) |
