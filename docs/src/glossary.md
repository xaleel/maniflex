...
[Custom Endpoints](advanced/actions.md).
...
interface. See [Database Backends](databases.md).
...
(`ctx.DBResult`, `ctx.Response`). See [Writing Middleware](middleware.md).
...
per-field `Changes` diff. See [Audit Logging](advanced/audit.md).
...
[Database Backends](databases.md).
...
[Models & BaseModel](models.md).

**Batch.** Multiple inserts or updates inside a single request, usually
through a [custom action](advanced/actions.md) that opens one transaction.
See [Batch Operations & Sagas](advanced/batch-saga.md).

**Before (position).** The default middleware position; the function runs
before the step's default handler. See [Writing Middleware](middleware.md).
...
`maniflex/middleware/`. See [Middleware Catalogue](middleware/index.md).
...
`Response`, `Auth`, `Tx`. See [ServerContext](context.md).
...
are excluded. See [Versioning & History](advanced/versioning.md).

## E

**Embed.** A `BaseModel`, `WithDeletedAt`, or `WithIsDeleted` value
included in a model struct as an anonymous field. Embeds contribute
columns and turn on framework behaviour. See [Models & BaseModel](models.md)
and [Soft Delete](soft-delete.md).
...
customises the spec. See [OpenAPI Spec](openapi.md).
...
the source model. See [Versioning & History](advanced/versioning.md).
...
`ServiceAccount`, or `Anonymous`. See [ServerContext](context.md).

**Immutable (tag).** A `mfx:` directive that accepts a value on create but
strips it on update. See [Field Tags Reference](tags.md).

**Include.** The `?include=relationKey,...` query parameter that populates
related rows inline in the response. See [Relations](relations.md) and
[Querying](querying.md).
...
`VaultKeyProvider`. See [Encryption at Rest](advanced/encryption.md).
...
Postgres; transaction-level lock on SQLite. See [Transactions](transactions.md).
...
model, routed through `ctx.Tx` when set. See [ServerContext](context.md).

**`ModelConfig`.** The per-model options passed alongside a struct in
`MustRegister`. Fields: `TableName`, `SoftDelete`, `Middleware`,
`Versioned`, `VersionedDiffOnly`, `Indices`. See [Models & BaseModel](models.md).
...
`cascade`, `setNull`, `restrict`, or unset. See [Relations](relations.md).

**Operation (`maniflex.Operation`).** The CRUD verb identifying a request:
`OpList`, `OpRead`, `OpCreate`, `OpUpdate`, `OpDelete`, `OpHead`,
`OpOptions`, `OpAction`. See [Pipeline Overview](pipeline.md).
...
[Raw Queries & Query Models](advanced/raw-queries.md).

**`QueryParams`.** The parsed `?page=&limit=&filter=&sort=&include=` of
a request, stored on `ctx.Query`. See [Querying](querying.md).
...
or `ManyToMany`. See [Relations](relations.md).
...
See [Writing Middleware](middleware.md).
...
worker. See [Batch Operations & Sagas](advanced/batch-saga.md).
...
[Scheduled Fields](advanced/scheduled.md).
...
automatically. See [Transactions](transactions.md).
...
See [Versioning & History](advanced/versioning.md).
