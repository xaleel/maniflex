...
See [Satellite Modules](modules.md) for the full layout and import rules.
...
or rewrite arbitrary fields. See [OpenAPI Spec](openapi.md) and the
[OpenAPI Middleware](middleware/openapi.md) catalogue.

A third, trimmed pipeline runs for [custom actions](advanced/actions.md):
...
| `mfx:"scheduled"` sweep | outside the request — separate runner | see [Scheduled Fields](advanced/scheduled.md) |
...
- **[Glossary](glossary.md)** — every framework term in one place.
- **[Pipeline Overview](pipeline.md)** — the per-step reference.
