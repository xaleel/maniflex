...
See [Transactions](transactions.md) for the underlying `ctx.LockForUpdate`
and `BeginTx` APIs.
...
See [Querying](querying.md) for the filter, sort, and cursor-pagination grammar.
...
documented in [Events & Background Jobs](advanced/events-jobs.md).
