...
- **One attachment path per `mfx:"file"` field** when storage is configured:
  ...
  [Per-model attachment routes](files.md#per-model-attachment-routes).
...
[Actions](advanced/actions.md) — custom endpoints registered with
...
[Documenting an action in OpenAPI](advanced/actions.md#documenting-an-action-in-openapi).
...
[OpenAPI Middleware](middleware/openapi.md) for the catalogue of
...
[`static/openapi.html`](static-files.md). When `static/` is present in the
...
[OpenAPI Middleware](middleware/openapi.md) helpers. For deeper edits, write
your own middleware:
