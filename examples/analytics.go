...
func printHelp(privateKey *rsa.PrivateKey) {
	...
	log.Println("  GET    /api/live                      liveness (process only)")
	log.Println("  GET    /api/ready                     readiness (DB ping + configured checks)")
	log.Println("  GET    /api/health                    legacy health check (DB ping)")
	log.Println()
