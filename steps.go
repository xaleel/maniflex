...
// strippedFromWrite reports whether this field's client-supplied value is
// removed before the write: the adapter's own id, a readonly field, or an
// immutable one on update.
//
// A value the *server* stamped is not from a client. Without that exception a
// middleware running before Validate — db.Tenancy hoisted with ProvidesScope, an
// auth middleware setting an owner column — had its injection stripped, and the
// row was written with the field empty: invisible to the tenant that created it,
// and visible to anyone whose scope is also empty.
func strippedFromWrite(ctx *ServerContext, f *FieldMeta) bool {
	if f.Tags.DBName == "id" {
		return true
	}
	jn := f.Tags.JSONName
	...
	return f.Tags.Immutable && ctx.Operation == OpUpdate && !ctx.ServerSetField(jn)
}

// stripClientWrites removes every value a client may not write from both the
// parsed body and the typed record.
//
// It is idempotent, which is what lets it run twice: once inside the Validate
// step, where it has always run, and once more in a fixed segment after that
// step that AtPosition(Replace) cannot remove. Replace swaps out a step's whole
// default handler, and this strip was the only thing enforcing mfx:"readonly",
// mfx:"immutable" and the generated id — so a one-line "custom validator"
// silently turned all three off and let a client set role, owner_id or the
// tenant column db.Tenancy stamps (audit PIPE-5). Whether the tags hold is not a
// matter of which validator an application prefers, the same way scoping is not
// a matter of which DB step it registers.
func stripClientWrites(ctx *ServerContext) {
	if ctx.Model == nil || (ctx.Operation != OpCreate && ctx.Operation != OpUpdate) {
		return
	}
	for i := range ctx.Model.Fields {
		f := &ctx.Model.Fields[i]
		if strippedFromWrite(ctx, f) {
			ctx.DeleteField(f.Tags.JSONName)
		}
	}
}
...
func (s *defaultSteps) validate(ctx *ServerContext, next func() error) error {
	...
	for _, field := range ctx.Model.Fields {
		...
		// Stripped above, and a value that is not being written has nothing left
		// to validate: a readonly field carrying a client's malformed value is
		// dropped, not a 422 naming a field they were never allowed to send.
		if strippedFromWrite(ctx, &field) {
			continue
		}
